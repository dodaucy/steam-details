...
function addGame(game, resultItem) {
    ...
    if (!game.services.steam_core.success) {  // This should never happen
        throw new Error(`Steam error: ${game.services.steam_core.error}`);
    }

    // Retry Button
    for (const service in game.services) {
        if (!game.services[service].success) {
            addRetryButton("Retry", game.services.steam_core.data.appid, resultItem);
            break;
        }
    }
    ...
    anchorWithImages.href = game.services.steam_core.data.external_url;
    anchorWithImages.target = "_blank";
    anchorWithImages.className = "images";

    Array.from(game.services.steam_core.data.images).forEach((image_url) => {
    ...
    titleAnchor.href = game.services.steam_core.data.external_url;
    titleAnchor.target = "_blank";
    titleAnchor.className = "title";
    titleAnchor.textContent = game.services.steam_core.data.name
